...
(**
  ...
*)
let test_bool (t_name_1:string) (t_name_2:string) (test_thing:bool) =
  t_name_1 >:: (fun _ -> assert_bool t_name_2 test_thing)
;;
...
(**
  ...
*)
let rec test_bools (num:int) (raise:string) (test_thing:bool list):OUnitTest.test list =
  match test_thing with
  | [] -> []
...
(**
  ...
*)
let run_test (test_name:string) (raise:string) (test_thing:bool list): unit =
  run_test_tt_main (test_name >::: test_bools 0 raise test_thing)
;;
...
(*
  TESTING ...   
*) 
run_test "indice_valide" "indice is not valise" [
...
(*
let tests_est_case = "est_case" >::: [
  (* i < -dim *)
  test_funny "01" "not a case" (est_case (-2 * dim, +1 * dim, +1 * dim));

  (* i > dim *)
  test_funny "02" "not a case" (est_case (+2 * dim, -1 * dim, -1 * dim));

  (* j < -dim*)
  test_funny "03" "not a case" (est_case (+1 * dim, -2 * dim, +1 * dim));

  (* (i,j,k) = (2dim, -dim, -dim) *)
  test_funny "04" "not a case" (est_case (+2 * dim, -1 * dim, -1 * dim));

  (* (i,j,k) = (-dim - 1, 1, dim) *)
  test_funny "05" "not a case" (est_case (-dim - 1, 1, dim));

  (* i >= -dim && j >= -dim && k >= -dim *)
  test_funny "06" "not a case" (est_case (-dim, -dim, 2*dim));
] ;;
*)
